...
void main() {
  ...
  group('Plugin template files', () {
    ...
    testUsingContext(
      'plugin class name conversion from snake_case',
      () {
        // Test the name conversion logic used in create.dart
        const pluginName = 'my_tvos_plugin';
    ...
    testUsingContext(
      'single word plugin name conversion',
      () {
  ...
  group('Plugin pubspec patching', () {
    testUsingContext(
      'adds tvOS platform to pubspec.yaml',
      () {
...
''');
    ...
    testUsingContext(
      'does not duplicate tvOS entry if already present',
      () {
        final File pubspecFile = fileSystem.file('/project/pubspec.yaml')
          ..createSync(recursive: true);
        pubspecFile.writeAsStringSync('''
