...
class NativeTvosBundle extends Target {
  ...
  /// "Embed & Sign" product on the Runner target), so an archive of such a
  /// project ships an *unsigned* `Flutter.framework`. That installs locally and
  /// via Internal TestFlight (Apple re-signs during processing) but is rejected
  /// by Beta App Review / the App Store with ITMS-91065 ("Missing signature") —
  /// Flutter is a commonly-used third-party SDK that must ship signed.
  ///
  /// Projects generated by the current template carry a "Sign Flutter.framework"
  /// run-script phase that re-signs the embedded engine with the app's own
  /// identity (like CocoaPods' embed script did). Older projects must regenerate
  /// (or add the phase) to archive successfully.
  void _warnIfFlutterFrameworkUnsigned(Directory tvosProjectDir) {
    ...
    globals.logger.printWarning(
      ...
      'build phase, so the Flutter engine embedded via Swift Package Manager is '
      'not code-signed. Device installs and Internal TestFlight still work '
      '(Apple re-signs during processing), but Beta App Review / the App Store '
      'reject the archive with ITMS-91065 ("Missing signature").\n'
