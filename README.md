...
- **On-device debug needs a debugger attached.** `flutter-tvos run -d <appletv> --debug` attaches lldb directly. Flutter 3.32.8's `flutter_tools` has no lldb attach API of its own, so this line vendors one; when Xcode has not yet finished preparing debugger support for the device's OS build, the CLI falls back to driving Xcode's debug action over OSA scripting — the same mechanism stock Flutter 3.32.8 uses for iOS Core Devices, and what copies those symbols in the first place. Xcode must be selected (`xcode-select -p`), and the fallback may ask for permission to control it (System Settings ▸ Privacy & Security ▸ Automation). **Open the device once in Xcode ▸ Window ▸ Devices and Simulators and wait for symbol preparation to finish**; until it does, every on-device debug run is slow. Because Apple TV is wireless-only the attach is slower than USB iOS; the **simulator remains the fast-iteration path** for debug. Release/profile on device run AOT and need no debugger.

  **Verified on hardware** (Apple TV 4K, tvOS 26.6): launch, hot reload and hot restart. This needs the current `v1.0.1-flutter3.32.8` debug engine — the artifact was replaced in place on 2026-09-01 to fix a Dart JIT failure on tvOS 18.4 and newer. If you precached before that date, run `rm -rf <flutter-tvos>/engine_artifacts && flutter-tvos precache` to pick it up; the stamp check will not re-download on its own.
